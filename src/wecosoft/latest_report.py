"""
Builds the PDF report for the "Activate Scraper" button: a simple table of
every product reference and its most recent CAAT listino price. No
taxonomy needed — this runs straight off the raw scraper output.
"""
...
def build_latest_report(scraper_output_xlsx: str, output_pdf: str) -> dict:
    ...
    latest = latest.rename(columns={latest_date: "prezzo"})
    latest = latest.dropna(subset=["prezzo"])
    latest = latest.sort_values("REFERENZA_COMPLETA").reset_index(drop=True)

    rows = [[cell(r["REFERENZA_COMPLETA"]), f"{r['prezzo']:.2f}"] for _, r in latest.iterrows()]
